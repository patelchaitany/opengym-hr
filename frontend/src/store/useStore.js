...
export const DEF = {
  ...
  // Heart rate, read live from the hr-bridge (../../hr-bridge) over Bluetooth.
  //   on      — capture heart rate during workouts at all
  //   url     — bridge address; empty means same-origin /hr, which is what the
  //             bundled nginx proxies, so a phone needs no address at all
  //   maxBpm  — measured maximum, if you know it. Null falls back to an
  //             age-based estimate, and the app labels the zones as estimated
  //   restBpm — resting rate, for the %-of-reserve and TRIMP maths
  //   age     — only used by the estimates that need it (max HR, calories)
  //   series  — keep the session's HR curve on the finished workout, so History
  //             can draw it back. ~3 KB a session; off for anyone who'd rather
