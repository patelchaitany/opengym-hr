...
import { startFlow, exercisePicker, exConfigSheet, exerciseDetailSheet, topWeightSheet, finishWorkout, workoutCompleteSheet, confirmSheet, heartRateSheet } from '../sheets.jsx'
import { useHR } from '../store/useHR.js'
import { HRStrip, HRExerciseLine } from '../components/HeartRate.jsx'
...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onStartTimed }) {
  ...
  return <>
    ...
    {/* What this exercise has done to your heart so far — the comparison that
        makes heart rate worth having in a lifting app lives next to the sets,
        not only in a summary at the end. */}
    <HRExerciseLine entry={entry} />
  </>
}
...
function ActiveWorkout() {
  ...
  const toggle = (idx, i) => {
    ...
    // The stretch of the session this set claims: from the moment work resumed
    // (the last rest ended, or the previous set was logged) to now. Only the
    // window is recorded, not a verdict — when the workout is finished the same
    // window is re-read against the bridge's own history, which is the copy that
    // survived the phone locking its screen mid-set. See analyzeWorkout.
    const nowMs = Date.now()
    const hrOn = !!S.hr?.on
    update(s => {
      const e = s.active.entries[idx]
      e.sets[i].done = !e.sets[i].done
      if (!e.sets[i].done && hrOn) {
        // Unticked — it is not a logged set any more, so it owns no window.
        delete e.sets[i].hrw; delete e.sets[i].hr
      }
      if (e.sets[i].done) {
        if (hrOn) {
          const from = s.active.hrMark || s.active.start
          e.sets[i].hrw = [from, nowMs]
          // A live snapshot so numbers appear the instant the box is ticked;
          // replaced by the authoritative pass when the workout ends.
          const st = useHR.getState().stats(from, nowMs)
          if (st.n) e.sets[i].hr = { avg: st.avg, max: st.max, min: st.min }
          s.active.hrMark = nowMs      // rest begins here, and so does the next set's window
        }
  ...
  // Rest ending is where the next set's work begins, so that is where its
  // heart-rate window starts. Without this the window would reach back across
  // the whole rest and report an average halfway between working and sitting
  // down — which would make every set look easier than it was.
  const resting = useUI(s => !!s.timer)
  useEffect(() => {
    if (resting || !S.hr?.on) return
    // push:false — a timestamp is not worth a round trip to the server; it rides
    // along with the next set that is logged.
    update(s => { if (s.active) s.active.hrMark = Date.now() }, false)
  ...
  return <div className="narrow">
    ...
    <HRStrip from={A.hrMark || A.start} onClick={heartRateSheet} />
