...
export default function Settings() {
  ...
  return <div className="narrow">
    ...
    <HeartRateCard S={S} update={update} toast={toast} />
...
/* ---------- heart rate ----------
   Everything about the strap and the numbers derived from it. Kept in one card
   rather than sprinkled through "During a workout" because it is a feature you
   either use or don't, and the ones who don't should be able to skip it whole.

   The address field is the one thing here that can go wrong in a way the app
   cannot diagnose on its own, so it comes with a button that actually calls the
   bridge and reports what it found, rather than a saved value and a shrug. */
function HeartRateCard({ S, update, toast }) {
  const hr = S.hr || {}
  ...
  const test = async () => {
    setBusy(true); setProbe(null)
    try {
      const info = await fetchInfo(draft)
      const dev = info.device?.name || info.device?.address
      setProbe({
        ok: true,
        text: info.status === 'streaming'
          ? t('Connected — reading {0}{1}.', dev || t('a device'), info.simulated ? ' ' + t('(simulated)') : '')
          : t('Bridge is running, but no heart-rate device is broadcasting yet ({0}).', info.status)
      })
    } catch (e) {
      setProbe({ ok: false, text: t('No answer from {0}. Is the bridge running?', httpBase(draft)) })
    } finally { setBusy(false) }
  }
  const saveUrl = () => update(s => { s.hr = { ...s.hr, url: draft.trim() } })
      ...
      {hr.on && !ble && <>
        <div className="lrow" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 10, paddingTop: 13, paddingBottom: 14 }}>
          ...
          <TextField value={draft} inputMode="url"
            placeholder={MOBILE ? 'http://192.168.1.20:3001' : t('same origin ({0})', DEFAULT_BASE)}
            autoCapitalize="off" autoCorrect="off" spellCheck="false"
            onChange={e => setDraft(e.target.value)} onBlur={saveUrl} />
          <div className="row" style={{ gap: 8 }}>
            <Button size="sm" icon="bolt" disabled={busy} onClick={async () => { saveUrl(); await test() }}>
              {busy ? t('Checking…') : t('Test connection')}
            </Button>
          </div>
          {probe && <div className={'small ' + (probe.ok ? 'accent' : '')} style={probe.ok ? null : { color: 'var(--red)' }}>{probe.text}</div>}
          <div className="small dim">{MOBILE
            ? t('The installed app has no server of its own, so this one has to be filled in: the address of the machine running the bridge, on the Wi-Fi you both share.')
            : t('Leave empty when openGym and the bridge are served together. On a phone against a laptop bridge, use http://<laptop-ip>:3001.')}</div>
        </div>
        <Row icon="chartLine" iconTint="var(--purple)" title={t('Keep the session curve')}
          subtitle={t('Draws your heart rate back in History. Adds ~3 KB per workout.')}>
          <Switch checked={hr.series !== false} onChange={v => update(s => { s.hr = { ...s.hr, series: v } })} />
        </Row>
      </>}
    </Section>

    {hr.on && <Section title={t('Your numbers')}
      footer={t('Zones, calories and strain are all derived from these. The closer they are, the more the numbers mean.')}>
      <Row icon="bolt" iconTint="var(--orange)" title={t('Max heart rate')}
        subtitle={maxHRIsEstimated(hr) ? t('Estimated: {0} bpm', maxHRFor(hr)) : t('Measured')}>
        <span className="stp-inline">
          <NumberField decimal={false} nullable value={hr.maxBpm ?? ''} placeholder={t('auto')}
            onChange={v => update(s => { s.hr = { ...s.hr, maxBpm: v } })} />
        </span>
      </Row>
      <Row icon="moon" iconTint="var(--indigo)" title={t('Resting heart rate')}>
        <span className="stp-inline">
          <NumberField decimal={false} nullable value={hr.restBpm ?? ''} placeholder="60"
            onChange={v => update(s => { s.hr = { ...s.hr, restBpm: v } })} />
        </span>
      </Row>
      <Row icon="person" iconTint="var(--blue)" title={t('Age')}
        subtitle={t('Only used for the max-HR and calorie estimates.')}>
        <span className="stp-inline">
          <NumberField decimal={false} nullable value={hr.age ?? ''} placeholder="—"
            onChange={v => update(s => { s.hr = { ...s.hr, age: v } })} />
        </span>
      </Row>
      <div className="lrow" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 8, paddingTop: 13, paddingBottom: 14 }}>
        <span className="lrow-t">{t('Your zones')}</span>
        <HRZoneTable />
      </div>
    </Section>}
  </>
