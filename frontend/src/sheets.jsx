...
import { analyzeWorkout, hrConfig } from './lib/hrmetrics.js'
import { HRSummary, HRZoneTable, HRWorkoutChart } from './components/HeartRate.jsx'
...
function WorkoutDetail({ w, close }) {
  ...
  return <>
    ...
    {w.hr && <>
      <HRWorkoutChart hr={w.hr} />
      <HRSummary hr={w.hr} />
      <div style={{ height: 6 }} />
    </>}
    {w.entries.map((e, i) => {
      const ex = EXIDX[e.id]
      // Positional lookup — the same exercise can appear twice in one session.
      const ehr = w.hr?.exercises?.find(x => x.i === i)?.hr
      return <div key={i} className="row" style={{ marginBottom: 12, alignItems: 'flex-start' }}>
        ...
        <div className="grow"><div className="tt capitalize" style={{ fontWeight: 600 }}>{ex ? ex.n : (e.n || e.id)} {w.prs && w.prs.includes(e.id) && <span className="pr"><Icon name="trophy" />PR</span>}</div>
          ...
          {ehr && <div className="ss hrline">
            <Icon name="heart" style={{ color: 'var(--red)' }} />
            {t('{0} avg · {1} peak', ehr.avg, ehr.max)}
            {ehr.kcal != null ? ' · ' + t('{0} kcal', ehr.kcal) : ''}
            {ehr.recovery?.drop > 0 ? ' · ' + t('−{0} in 60s', ehr.recovery.drop) : ''}
          </div>}
        </div>
      </div>
    })}
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  const st = useStore(s => s.S)
  // The heart-rate passes land after this sheet is already open (and one of
  // them a minute later), so read the stored copy rather than the snapshot the
  // sheet was opened with — the numbers fill themselves in.
  const saved = st.workouts.find(x => x.id === w.id) || w
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    {saved.hr && <div style={{ textAlign: 'left' }}>
      <h4 className="sec">{t('Heart rate')}</h4>
      <HRWorkoutChart hr={saved.hr} h={110} />
      <HRSummary hr={saved.hr} />
    </div>}
...
// Tapping the live strip mid-workout. Answers the two questions worth asking
// with a bar in your hands: is this thing actually reading me, and what does
// the colour I'm looking at mean.
export function heartRateSheet() {
  ui().openSheet(close => <HeartRatePanel close={close} />)
}
function HeartRatePanel({ close }) {
  ...
  return <>
    ...
    <div className="row" style={{ gap: 14, alignItems: 'baseline', marginBottom: 4 }}>
      <div style={{ fontSize: 40, fontWeight: 700, letterSpacing: '-.03em', color: live ? 'var(--red)' : 'var(--label-3)' }}>
        {live ? bpm : '—'}<span style={{ fontSize: 15, fontWeight: 500, color: 'var(--label-2)' }}> bpm</span>
      </div>
      {live && <div className="muted">{Math.round(bpm / cfg.maxBpm * 100)}% {t('of max')} · {samples.length} {t('readings')}</div>}
    </div>
    <div className="muted small" style={{ marginBottom: 14 }}>{line}</div>
    {battery != null && <div className="small dim" style={{ marginBottom: 12 }}>{t('Device battery')}: {battery}%</div>}
    <h4 className="sec">{t('Your zones')}</h4>
    <HRZoneTable />
    <div style={{ height: 12 }} />
    <Button icon="gear" onClick={() => { close(); nav('/settings') }}>{t('Heart-rate settings')}</Button>
    <div style={{ height: 8 }} />
...
/* ---------------------------------------------------------- heart rate ----
   Three passes, because the right answer isn't available all at once.

   1. Immediately, from whatever this tab received: the finish sheet has real
      numbers the moment it opens, and a workout finished with the bridge
      already gone still keeps them.
   2. Straight after, from the bridge's own history: it kept every beat,
      including the ones that never reached a phone with its screen off, so
      this is the copy that is actually right.
   3. Seventy seconds later, once the minute after the final set has elapsed —
      the only way the last exercise gets a recovery figure, since when the set
      was ticked that minute hadn't happened yet.

   All of it is fire-and-forget. The workout is saved before any of this runs;
   a bridge that has been unplugged must never be able to fail a finish. */
function captureHeartRate(w) {
  const st = S()
  if (!st.hr?.on) return
  const hrs = useHR.getState()
  const cfg = hrConfig(st, w)
  const base = st.hr.url || ''
  const pad = 5000                      // a little either side, so the edges aren't clipped

  applyHR(w.id, analyzeWorkout(w, hrs.samples, cfg))

  const refine = () => hrs.backfill(base, w.start - pad, Date.now())
    .then(series => applyHR(w.id, analyzeWorkout(w, series, cfg)))
    .catch(() => { /* offline — pass 1 stands */ })

  refine()
  setTimeout(refine, 70000)
}

function applyHR(id, hr) {
  if (!hr) return
  update(s => {
    const w = s.workouts.find(x => x.id === id)
    if (!w) return                       // deleted from History while we were away
    // Per-set numbers go on the sets, where every existing reader (the set
    // label, History, a CSV export) already looks — not into a parallel
    // structure that each of them would have to learn about.
    hr.exercises.forEach(ex => {
      const we = w.entries[ex.i]
      if (!we || !ex.hr?.sets) return
      we.sets.forEach((set, i) => {
        const h = ex.hr.sets[i]
        if (h) set.hr = { avg: h.avg, max: h.max, min: h.min }
      })
      delete ex.hr.sets                  // now stored on the sets themselves
    })
    // The curve is the one big thing here (~3 KB), and the whole profile is
    // re-uploaded whenever anything changes, so it is opt-out.
    if (!s.hr?.series) delete hr.series
    w.hr = hr
  })
}
